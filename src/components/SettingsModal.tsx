...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleToggleAutoDriveBackup = (checked: boolean) => {
    setStorageConfig(prev => ({ ...prev, autoDriveBackup: checked }));
    localStorage.setItem('cns_auto_drive_backup', String(checked));
    onAddToast(
      checked
        ? "Đã bật tự động sao lưu ngầm cơ sở dữ liệu lên thư mục 'QLVT_Backup' trên Google Drive."
        : "Đã tắt tự động sao lưu ngầm Google Drive.",
      checked ? 'success' : 'info'
    );
  };
          ...
          {activeTab === 'STORAGE' && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="space-y-2.5">
                ...
                {/* Google Drive Integration & Auto QLVT_Backup Card */}
                <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-50/80 via-white to-slate-50 dark:from-emerald-950/40 dark:via-slate-800/60 dark:to-slate-900 border border-emerald-200 dark:border-emerald-800/60 space-y-3 shadow-xs">
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5 pr-2">
                      <span className="text-xs font-black text-slate-800 dark:text-white flex items-center gap-1.5">
                        <HardDrive className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
                        Tự động sao lưu ngầm lên Google Drive ('QLVT_Backup')
                      </span>
                      <p className="text-[10.5px] text-slate-500 dark:text-slate-400 leading-relaxed">
                        Tự động đẩy bản sao lưu cơ sở dữ liệu vật tư lên thư mục chuyên dụng <strong>QLVT_Backup</strong> trên Google Drive.
                      </p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer shrink-0">
                      <input
                        type="checkbox"
                        checked={storageConfig.autoDriveBackup !== false}
                        onChange={(e) => handleToggleAutoDriveBackup(e.target.checked)}
                        className="sr-only peer"
                      />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10.5px] pt-1 border-t border-emerald-100 dark:border-slate-700/60">
                    <div className="p-2.5 bg-white dark:bg-slate-800/90 rounded-xl border border-slate-150 dark:border-slate-700">
                      <span className="text-slate-400 block mb-0.5 font-medium">Lần sao lưu Drive gần nhất:</span>
                      <strong className="text-slate-800 dark:text-slate-200 font-mono text-[11px] block truncate">
                        {formatLastBackup(storageConfig.lastDriveBackupTime)}
                      </strong>
                    </div>
                    <div className="p-2.5 bg-white dark:bg-slate-800/90 rounded-xl border border-slate-150 dark:border-slate-700">
                      <span className="text-slate-400 block mb-0.5 font-medium">Thư mục đích Google Drive:</span>
                      <strong className="text-emerald-600 dark:text-emerald-400 font-mono text-[11px] block truncate">
                        /QLVT_Backup
                      </strong>
                    </div>
                  </div>
              ...
              <div className="pt-2 flex flex-col sm:flex-row gap-2.5">
                ...
                <button
                  ...
                  title="Làm mới bộ nhớ kho thiết bị"
                >
                  <RotateCcw className="w-3.5 h-3.5" />
                  <span>Làm mới bộ nhớ kho</span>
                </button>
              </div>
            </div>
          )}
          ...
          {activeTab === 'CLOUD' && (
            <div className="space-y-4 animate-fade-in">
              ...
              <div>
                ...
                {syncConfig.webAppUrl.trim().endsWith('/edit') && (
                  <div className="mt-2 p-2.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl flex items-center justify-between gap-2 text-xs text-amber-800 dark:text-amber-300">
                    <div className="flex items-center gap-2">
                      <AlertTriangle className="w-4 h-4 shrink-0 text-amber-600" />
                      <span className="text-[11px] font-medium">Đường dẫn kết thúc bằng <code>/edit</code>. Cần đổi thành <code>/exec</code> để gọi API.</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => {
                        const fixed = syncConfig.webAppUrl.trim().replace(/\/edit.*$/, '/exec');
                        saveSettingsConfig(fixed);
                        onAddToast('Đã tự động sửa đuôi URL thành /exec', 'success');
                      }}
              ...
              {/* Collapsible Google Apps Script Code Template */}
              <div className="border border-slate-200 dark:border-slate-700/80 rounded-2xl overflow-hidden bg-slate-50/70 dark:bg-slate-800/30">
                <button
                  type="button"
                  onClick={() => setShowScriptCode(!showScriptCode)}
                  className="w-full px-3.5 py-3 flex items-center justify-between text-left hover:bg-slate-100/70 dark:hover:bg-slate-800/60 transition-colors cursor-pointer"
                >
                  <div className="flex items-center gap-2">
                    <Code className="w-4 h-4 text-indigo-600 dark:text-indigo-400 shrink-0" />
                    <div>
                      <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block">
                        Mã Google Apps Script mẫu (Code.gs) & Hướng dẫn
                      </span>
                      <span className="text-[10px] text-slate-400 block">
                        Sao chép mã mẫu dán vào script.google.com để kết nối Google Sheet
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 shrink-0 ml-2">
                    <span>{showScriptCode ? 'Thu gọn' : 'Xem mã'}</span>
                    <ChevronDown className={`w-3.5 h-3.5 transition-transform ${showScriptCode ? 'rotate-180' : ''}`} />
                  </div>
                </button>

                {showScriptCode && (
                  <div className="p-3.5 border-t border-slate-200/80 dark:border-slate-700/80 space-y-3 bg-white dark:bg-slate-900/60">
                    <div className="text-[11px] text-slate-600 dark:text-slate-300 space-y-1">
                      <p className="font-bold text-slate-800 dark:text-white">Cách thiết lập Google Sheet trong 1 phút:</p>
                      <ol className="list-decimal list-inside space-y-0.5 text-[10.5px] text-slate-500 dark:text-slate-400">
                        <li>Mở Google Sheet mới hoặc hiện có của Đội Thông Tin.</li>
                        <li>Vào menu <b>Tiện ích mở rộng (Extensions) &gt; Apps Script</b>.</li>
                        <li>Dán toàn bộ mã dưới đây vào file <b>Code.gs</b> rồi nhấn Lưu (Ctrl+S).</li>
                        <li>Nhấn <b>Triển khai (Deploy) &gt; Quản lý triển khai mới (New deployment)</b>.</li>
                        <li>Chọn loại <b>Ứng dụng web (Web app)</b>, mục <i>Ai có quyền truy cập</i> chọn <b>Bất kỳ ai (Anyone)</b>.</li>
                        <li>Sao chép URL kết thúc bằng <code>/exec</code> và dán vào ô bên trên.</li>
                      </ol>
                    </div>
