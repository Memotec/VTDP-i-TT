...
import {
  ...
  Mail, KeyRound, ShieldCheck, Sparkles, ExternalLink, Eye, EyeOff, LogIn
} from 'lucide-react';
...
export default function App() {
  ...
  // Google / Gmail account sign in handler
  const handleGoogleAccountLogin = async () => {
    try {
      setIsLoggingInGoogle(true);
      setLoginError('');
      const authResult = await signInWithGoogleAccount();
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] dark:bg-[#0B0F19] text-slate-900 dark:text-slate-100 flex flex-col antialiased selection:bg-blue-600 selection:text-white">
      ...
      {/* NOT LOGGED IN SCREEN - STREAMLINED & CLEAN */}
      {!role ? (
        <div className="flex-1 flex flex-col items-center justify-center p-4 sm:p-6 bg-slate-900 dark:bg-slate-950 min-h-screen">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-3xl shadow-2xl border border-slate-200/80 dark:border-slate-800 animate-scale-in">
            {/* Brand Header */}
            <div className="text-center mb-6">
              <div className="w-13 h-13 bg-gradient-to-tr from-blue-600 via-indigo-600 to-blue-700 rounded-2xl flex items-center justify-center mx-auto mb-3 shadow-md shadow-blue-500/25">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5 uppercase tracking-wider ml-0.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10.5 pr-9 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-slate-900 dark:text-white font-medium outline-none focus:ring-3 focus:ring-blue-500/15 focus:border-blue-500 transition-all text-sm placeholder:text-slate-400"
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5 ml-0.5">
                  ...
                  {username.toLowerCase().includes('@gmail.com') && (
                    <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold">
                      (Không bắt buộc với Gmail)
                    </span>
                  )}
                </div>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10.5 pr-10 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 text-slate-900 dark:text-white font-medium outline-none focus:ring-3 focus:ring-blue-500/15 focus:border-blue-500 transition-all text-sm placeholder:text-slate-400"
                  ...
                  <button
                    ...
                    className="absolute right-3 top-3.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer p-0.5"
                    tabIndex={-1}
                  >
              ...
              {loginError && (
                <div className="bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 px-3.5 py-2.5 rounded-xl text-xs font-semibold border border-rose-200 dark:border-rose-900/50 flex items-center gap-2">
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-xl shadow-md shadow-blue-600/20 hover:shadow-blue-600/30 transition-all text-sm tracking-wide active:scale-[0.98] cursor-pointer flex items-center justify-center gap-2"
              >
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                onClick={() => {
                  ...
                  localStorage.removeItem('cns_logged_out');
                  addToast('Xin chào Quản trị viên! Đăng nhập Super Admin thành công.', 'success');
                }}
                className="p-2.5 bg-amber-50/80 hover:bg-amber-100 dark:bg-amber-950/30 dark:hover:bg-amber-900/40 border border-amber-200 dark:border-amber-800/60 rounded-xl text-amber-900 dark:text-amber-200 text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer active:scale-95"
              >
              ...
              <button
                ...
                className="p-2.5 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-200 text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer active:scale-95 disabled:opacity-60"
              >
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 text-center">
              <p className="text-[11px] text-slate-400 dark:text-slate-500">
                Tài khoản mẫu: <span className="font-mono font-bold text-slate-600 dark:text-slate-300">admin / admin</span> • <span className="font-mono font-bold text-slate-600 dark:text-slate-300">guest / 123456</span>
              </p>
            </div>
          </div>
        </div>
